<template>
    <div>
        ...
        <p class="form-control-static" v-else-if="items && displayMode === 'VIEW' && typeof items[0] === 'object' && (property.filter || options.filter)">
            {{ $options.filters[options.filter || property.filter](displayFromObject) || '-' }}
        </p>
    ...
    methods: {
        ...
        handler() {
            ...
            if (this.options.fullObject) {
                ...
                clonedValue.value = this.selected && this.selected[this.options.valueField || '_id'] ? this.selected[this.options.valueField || '_id'] : this.selected;
            }
        ...
        selectFrom: {
            handler: function(newVal) {
                if (newVal.length > 0 && typeof newVal[0] === 'object') {
                    this.selected = newVal.find(item => item[this.options.valueField || '_id'] === this.selected);
